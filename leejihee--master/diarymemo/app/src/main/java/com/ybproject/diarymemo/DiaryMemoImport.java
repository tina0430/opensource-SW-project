...
public class DiaryMemoImport extends Activity {
	private static final String[] PROJECTION = new String[] { 
		DiaryMemo._ID, DiaryMemo.TITLE, DiaryMemo.BODY
	};
	...
	protected void onResume() {
		...
		if (action.equals("com.ybproject.diarymemo.action.ACTION_IMPORT")) {
		...
		public Integer doInBackground(Void... params) {
			...
			try {
				...
				for (int i = 0; i < count; i++) {
					...
					values.put(DiaryMemo.BODY, body);
					
					Cursor cursor = managedQuery(
						DiaryMemo.CONTENT_URI, PROJECTION, DiaryMemo.TITLE + "=?", new String[] { title }, DiaryMemo.DEFAULT_SORT_ORDER
					);
					...
					if ((cursor.getCount() == 1) && cursor.moveToFirst()) {
						...
						getContentResolver().insert(DiaryMemo.CONTENT_URI, values);
					}
					cursor.close();
